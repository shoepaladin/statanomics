"""
2x2 Monte Carlo: DiD vs SC (DI) vs SDID
DGP family: Y_it = alpha_i + 0.3*t + lambda_i*F_t + eps_it
"""
...
import panelib
from panelib import did, sdid, dgp as panelib_dgp, alpha_lambda
_di = panelib.di
...
ATT_PCT  = 0.02
N_SEEDS  = 100
DATA_DICT = {'treatment':'treated','date':'time','post':'post',
             'unitid':'unit_id','outcome':'y'}


def make_panel(seed, T_pre, N_control, sigma_lambda):
    np.random.seed(seed)
    N_total  = N_control + 1
    ...
    for i, uid in enumerate(unit_ids):
        ...
        for t in range(T_total):
            ...
            if is_treated and t == T_pre:
                y_cf = y
            rows.append({'unit_id':uid,'time':t,'y_base':y,
...
def sc_att_fast(df):
    pre = panelib_dgp.clean_and_input_data(
        dataset=df, treatment='treated', unit_id='unit_id',
        date='time', post='post', outcome='y')
    ptt = panelib_dgp.determine_pre_train_test_lengths(
        ci_data_output=pre, pre_train_test_lengths=None)
    y = pre['T_pre'].values.flatten()
    X = pre['C_pre'].values
    cv = ElasticNetCV(l1_ratio=[0.1,0.5,0.9,1.0], cv=3,
                      max_iter=10000, random_state=42)
    cv.fit(X, y)
    ba = float(cv.alpha_)
    bl = float(np.clip(cv.l1_ratio_, 1e-6, 1-1e-6))
    class _R:
        x = np.array([1000.*np.log(ba), np.log(bl/(1.-bl))])
    al = alpha_lambda.alpha_lambda_transform(_R().x)
    est = _di.predict_mu_omega(pre['T_pre'], pre['C_pre'], al, ptt)
    out = _di.sc_style_results(pre['T_pre'], pre['T_pst'],
                               pre['C_pre'], pre['C_pst'],
                               est['mu'], est['omega'])
    return float(out['atet'].mean(axis=0).mean())
...
def run_mc(T_pre, N_control, sigma_lambda, label):
    ...
    for seed in range(N_SEEDS):
        ...
        df, true_att = make_panel(seed, T_pre, N_control, sigma_lambda)
...
for name, cfg in DGP_CONFIGS.items():
    ...
    print(f"  sigma_lambda={cfg['sigma_lambda']}  T_pre={cfg['T_pre']}  N_control={cfg['N_control']}", flush=True)
    ...
    for m, c in [('DiD','did'),('SC (DI)','sc'),('SDID','sdid')]:
        ...
        print(f"  {m:8s}: mean_bias={b.mean():+.4f}  std={b.std():.4f}  rmse={np.sqrt((b**2).mean()):.4f}", flush=True)
...
print("="*70)
pivot_bias = summary_df.pivot_table(index='DGP', columns='Model', values='Mean_bias')
pivot_bias = pivot_bias[['DiD','SC (DI)','SDID']].reindex(list(DGP_CONFIGS.keys()))
print(pivot_bias.round(4).to_string())

print("\n" + "="*70)
print("  RMSE")
print("="*70)
pivot_rmse = summary_df.pivot_table(index='DGP', columns='Model', values='RMSE')
pivot_rmse = pivot_rmse[['DiD','SC (DI)','SDID']].reindex(list(DGP_CONFIGS.keys()))
print(pivot_rmse.round(4).to_string())

print("\n" + "="*70)
print("  FULL DETAIL")
print("="*70)
cols = ['DGP','Model','True_ATT_mean','Mean_ATT','Mean_bias','Std','Variance','IQR_lo','IQR_hi','IQR_width','RMSE']
print(summary_df[cols].round(4).to_string(index=False))

# Save
out = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'mc_2x2_results.csv')
