...
if test_out==True:
    N = 1000
    df,exp = generate_data(N, False, 2,2,
                                 linear_treatment,linear_outcome, linear_hte, 
                                 None,linear) 
    print('Treatment: {0}'.format(exp[0]))
    print('Outcome: {0}'.format(exp[1]))
    print('HTE: {0}'.format(exp[2])) 
    
    full_feature_list = [ x for x in df.columns if 'x' in x]
    
    ## 3. Run through each combination of models
    SGCT = het_dml_approaches.SGCT(df, 
    ...
    dr = other.DR(df, 
                    ...
                    full_feature_list,
                    y_models['ols'], t_models['logit'],
                   n_data_splits, {'force_second_stage':None})
    df['DR_TE'] = dr[0]
    df['DR_SE'] = dr[1]    
    df['DR_P'] = dr[2]
    df['DR_Y0'] = dr[3]
    ...
    dr_ols = other.DR(df, 
                    ...
                    full_feature_list,
                    y_models['ols'], t_models['logit'],
                   n_data_splits, {'force_second_stage':'OLS'})
    df['DR_OLS_TE'] = dr_ols[0]
    df['DR_OLS_SE'] = dr_ols[1]    
    df['DR_OLS_P'] = dr_ols[2]
    df['DR_OLS_Y0'] = dr_ols[3]
...
for r in [x for x in df.columns if x[-2:]=='_P']:
    a =  gml.blp(df, 
                    full_feature_list, 'Y','T',
                    r, r[0:-2]+'_TE', r[0:-2]+'_Y0',
                   5)
    
    print('{0} beta1: {1:5.3f}  pvalue1: {2:5.3f} | beta2: {3:5.3f}  pvalue2: {4:5.3f} '.format(r, a['beta1'], a['pvalue1_adj'],a['beta2'], a['pvalue2_adj'] ))
...
# In[31]:
    ...
    '''
    Scatterplot of true and predicted treatment effects
    '''

    fig, ax = plt.subplots(nrows=2,ncols=4, figsize=(20,10), sharey=True, sharex=True)
    ylim_min, ylim_max = np.min(df['GT']),np.max(df['GT'])
    
    ax[0,0].scatter(df['GT'], df['dml_HR_TE'], color='skyblue', alpha=0.5)
    ax[0,0].plot(df['GT'],df['GT'], color='black')
    ...
    ax[0,1].scatter(df['GT'], df['dml_SGCT_TE'], color='green', alpha=0.5)
    ax[0,1].plot(df['GT'],df['GT'], color='black')
    ax[0,1].set_xlabel('Ground Truth')
    ax[0,1].set_ylabel('Estimate')
    ax[0,1].grid(True)
    ax[0,1].set_title("SGCT")
    ax[0,1].set_ylim([ylim_min, ylim_max])
    
    ax[0,2].scatter(df['GT'], df['OLS_TE'], color='magenta', alpha=0.5)
    ax[0,2].plot(df['GT'],df['GT'], color='black')
    ax[0,2].set_xlabel('Ground Truth')
    ax[0,2].set_ylabel('Estimate')
    ax[0,2].grid(True)
    ax[0,2].set_title("OLS")
    ax[0,2].set_ylim([ylim_min, ylim_max])
    
    ax[0,3].scatter(df['GT'], df['GRF_TE'], color='coral', alpha=0.5)
    ax[0,3].plot(df['GT'],df['GT'], color='black')
    ax[0,3].set_xlabel('Ground Truth')
    ax[0,3].set_ylabel('Estimate')
    ax[0,3].grid(True)
    ax[0,3].set_title("GRF")
    ax[0,3].set_ylim([ylim_min, ylim_max])
    

    ax[1,0].scatter(df['GT'], df['DR_CVL_TE'], color='grey', alpha=0.5)
    ax[1,0].plot(df['GT'],df['GT'], color='black')
    ax[1,0].set_xlabel('Ground Truth')
    ax[1,0].set_ylabel('Estimate')
    ax[1,0].grid(True)
    ax[1,0].set_title("CVLasso DR")
    ax[1,0].set_ylim([ylim_min, ylim_max])

    ax[1,1].scatter(df['GT'], df['DR_OLS_TE'], color='teal', alpha=0.5)
    ax[1,1].plot(df['GT'],df['GT'], color='black')
    ax[1,1].set_xlabel('Ground Truth')
    ax[1,1].set_ylabel('Estimate')
    ax[1,1].grid(True)
    ax[1,1].set_title("OLS DR")
    ax[1,1].set_ylim([ylim_min, ylim_max])

    ax[1,2].scatter(df['GT'], df['DR_TE'], color='pink', alpha=0.5)
    ax[1,2].plot(df['GT'],df['GT'], color='black')
    ax[1,2].set_xlabel('Ground Truth')
    ax[1,2].set_ylabel('Estimate')
    ax[1,2].grid(True)
    ax[1,2].set_title("DR")
    ax[1,2].set_ylim([ylim_min, ylim_max])  
    fig.patch.set_facecolor('white')
    fig.suptitle('Scatterplot of Ground Truth\n and Predicted Treatment Effect')
